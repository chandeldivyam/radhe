...
function setDragImage(
	...
	const dragImageContainer = createDragImage(
		editor,
		draggableBlockElem,
		selectedBlocks
	);
	...
	// Set as drag image
	dataTransfer.setDragImage(dragImageContainer, 0, 0);
...
function validateDropData(data: unknown): DragData | null {
	...
	try {
		...
		if (
			!parsed.sourceKeys.every((key: unknown) => typeof key === 'string')
		) {
...
function useDraggableBlockMenu(
	...
	function onDragStart(event: ReactDragEvent<HTMLDivElement>): void {
		...
		if (!dataTransfer || !draggableBlockElem) {
			console.log(
				'Drag start failed: No dataTransfer or draggableBlockElem'
			);
			return;
		}
		...
		editor.update(() => {
			...
			const sourceKeys =
				selectedBlocks.size > 0 && selectedBlocks.has(primaryKey)
			...
			dataTransfer.setData(DRAG_DATA_FORMAT, JSON.stringify(dragData));
