...
export function NoteTree({ width }: NoteTreeProps) {
	const {
		rootNotes,
		childrenMap,
		expandedNodes,
	...
	const renderNoteTree = useCallback(
		(notes: NoteListItem[], level: number = 0) => {
	...
	const handleDragEnd = async (event: DragEndEvent) => {
		...
		const dropType = (over.id as string).split('-').pop() as
			| 'before'
			| 'inside';
		const overId = (over.id as string).slice(0, -dropType.length - 1);

		const sourceNote =
			rootNotes.find((n) => n.id === active.id) ||
			Object.values(childrenMap)
				.flat()
				.find((n) => n.id === active.id);
		const targetNote =
			rootNotes.find((n) => n.id === overId) ||
			Object.values(childrenMap)
				.flat()
				.find((n) => n.id === overId);
	...
	return (
		<DndContext onDragEnd={handleDragEnd}>
			<ScrollArea style={{ width }} className="h-[calc(100vh-8rem)]">
				<div className="min-h-full">{renderedTree}</div>
			</ScrollArea>
		</DndContext>
	);
}
